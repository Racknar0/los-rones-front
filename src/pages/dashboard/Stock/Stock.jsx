...
import { confirmAlert, errorAlert, successAlert, warningAlert } from '../../../helpers/alerts';
...
const Stock = () => {
  ...
  const simulateCreateStock = () => {
    ...
    if (selectedProduct?.perishable) {
      if (!expirationDates || expirationDates.length < quantity || expirationDates.some(date => !date || date.trim() === '')) {
        console.error("Error: El producto perecedero requiere fechas de vencimiento para cada unidad.");
  ...
  const openStockModal = async () => {
    ...
    try {
      ...
      const response = await httpService.getData(`/stock/${selectedProduct.id}/stockunits?storeId=${selectedStore}`);
      if (response.status === 200) {
  ...
  const handleBulkDelete = async () => {
    if (selectedStockIds.length === 0) {
      warningAlert('Advertencia', 'Seleccione al menos una unidad de stock para eliminar');
      return;
    }
    ...
    try {
      const response = await httpService.deleteDataWithBody('/stock', { ids: selectedStockIds });
      if (response.status === 200) {
        successAlert('Éxito', 'Unidades de stock eliminadas exitosamente');
  ...
  return (
    <div className="container-fluid mt-4 main_container">
      ...
      <Modal show={showModal} onClose={() => setShowModal(false)} title="Gestionar Stock">
        <StockTable 
